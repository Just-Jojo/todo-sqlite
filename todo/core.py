...
log = logging.getLogger("red.JojoCogs.todo.core")
...
class ToDo(
    ...
    async def todo(self, ctx: commands.Context, index: TodoPositiveInt):
        ...
        act_index = index - 1
        try:
            todo = await self.cache.get_user_item(ctx.author, "todos")
            todo = todo[act_index] # type:ignore
        except IndexError:
